...
class CompleteAttributionProcessor {
  ...
  // Waterfall attribution with proper pipeline separation
  async processWaterfallAttribution() {
    ...
    console.log('🔍 DEBUG: Starting Waterfall Attribution');
    console.log('📊 Total clients to process:', this.clients.length);
    console.log('📊 V1 contact stats:', this.v1ContactStats.length);
    console.log('📊 V2 contact stats:', this.v2ContactStats.length);
    console.log('📊 V3 contact stats:', this.v3ContactStats.length);
      ...
      if (matchingContact) {
        ...
        for (const attempt of outreachAttempts) {
          ...
          if (daysDiff >= 1 && daysDiff <= 90) {
            clientsWithValidTiming++;
            console.log(`✅ Valid attribution: ${client.email}, days diff: ${daysDiff}`);
            const variantData = AttributionUtils.determineEmailVariant({ 
    ...
    console.log('📊 Direct Salesforce Attribution Results:');
    console.log(`   Total clients: ${totalClients}`);
    console.log(`   Clients with matching contacts: ${clientsWithContacts}`);
    console.log(`   Clients with outreach attempts: ${clientsWithOutreach}`);
    console.log(`   Clients with valid timing: ${clientsWithValidTiming}`);
  ...
  async processEnhancedInstagramAttribution() {
    const matches = [];
    let totalClients = 0;
    let clientsWithSpotifyId = 0;
    let clientsWithMatchingLeads = 0;
    
    console.log('🔍 DEBUG: Starting Instagram Attribution');
    console.log('📊 Total clients to process:', this.clients.length);
    console.log('📊 Total Convrt leads available:', this.convrtLeads.length);
    
    for (const client of this.clients) {
      totalClients++;
      if (!client.spotify_id) continue;
      clientsWithSpotifyId++;
      ...
      if (matchingLead) {
        clientsWithMatchingLeads++;
        console.log(`✅ Instagram match found: ${client.email} -> ${matchingLead.handle}`);
    ...
    console.log('📊 Instagram Attribution Results:');
    console.log(`   Total clients: ${totalClients}`);
    console.log(`   Clients with Spotify ID: ${clientsWithSpotifyId}`);
    console.log(`   Clients with matching leads: ${clientsWithMatchingLeads}`);
    console.log(`   Final matches: ${matches.length}`);
  ...
  async processEnhancedAuditAttribution() {
    ...
    let totalClients = 0;
    let clientsWithSpotifyId = 0;
    let clientsWithMatchingAudits = 0;
    
    console.log('🔍 DEBUG: Starting Audit Attribution');
    console.log('📊 Total clients to process:', this.clients.length);
    console.log('📊 Total audits available:', this.audits.length);
    ...
    for (const client of this.clients) {
      ...
      if (matchingAudit) {
        clientsWithMatchingAudits++;
        console.log(`✅ Audit match found: ${client.email} -> ${matchingAudit.artist_name}`);
    ...
    console.log('📊 Audit Attribution Results:');
    console.log(`   Total clients: ${totalClients}`);
    console.log(`   Clients with Spotify ID: ${clientsWithSpotifyId}`);
    console.log(`   Clients with matching audits: ${clientsWithMatchingAudits}`);
    console.log(`   Final matches: ${matches.length}`);
  ...
  async generateComprehensiveReport(
    ...
    console.log('🔍 DEBUG: Final Attribution Report Generation');
    console.log('📊 Total attributions found:', allAttributions.length);
    console.log('📊 Direct Salesforce matches:', directSalesforceMatches.length);
    console.log('📊 Instagram matches:', instagramMatches.length);
    console.log('📊 Audit matches:', auditMatches.length);
    console.log('📊 Invitation matches:', invitationMatches.length);
    ...
    console.log('📊 Final Attribution Counts:', attributionCounts);
    console.log('📊 Final Revenue Counts:', revenueCounts);
  ...
  try {
    switch (type) {
      case 'PROCESS_ATTRIBUTION':
        console.log('Starting attribution processing...');
        ...
        console.log('Processing attribution with data sources:', Object.keys(data));
